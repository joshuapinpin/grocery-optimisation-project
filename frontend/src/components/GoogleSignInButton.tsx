...
function GoogleSignInButton({ label = 'Continue with Google' }: GoogleSignInButtonProps) {
    function handleClick() {
        // full page redirect — this is NOT an API call, the browser needs to
        // actually leave the app and go through Google's login flow
        window.location.href = `${BACKEND_URL}/oauth2/authorization/google`
    }

    return (
        <button type="button" className="google-signin-btn" onClick={handleClick}>
            <svg className="google-signin-icon" viewBox="0 0 18 18" width="18" height="18">
